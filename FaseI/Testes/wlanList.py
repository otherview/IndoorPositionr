...
class DOT11_MAC_ADDRESS(Structure):
    _fields_ = [
        ("strInterfaceDescription", c_wchar * 6)
        ]
...
def get_rssi():
            ...
            try:
                ...
                for network in networks:
                    print "SSID: %s, quality: %02d%%" % (
                        ...
                        network.wlanSignalQuality)+ " : " + str(-100 + (network.wlanSignalQuality/2))+ " Dbm"
                ##Mycode###
                    if not power_listings.get(network.dot11Ssid.SSID[:network.dot11Ssid.SSIDLength]):
                        power_listings[network.dot11Ssid.SSID[:network.dot11Ssid.SSIDLength]] = [int(-100 + (network.wlanSignalQuality/2))]
                    else:
                        power_listings[network.dot11Ssid.SSID[:network.dot11Ssid.SSIDLength]].append(int(-100 + (network.wlanSignalQuality/2)))
                ##MC###
    ...
    print power_listings
    return power_listings
...
if __name__ == '__main__':
    ...
    try:
        ...
        for i in range(1,30):
            ...
            tmp_listings_dbm = get_rssi()
            for rssi in tmp_listings_dbm:
                if not listings_dbm.has_key(rssi):
                    listings_dbm[rssi] = [tmp_listings_dbm[rssi]]
                else:
                    listings_dbm[rssi].append(tmp_listings_dbm[rssi])
           # print "\n\n"
           # print listings_dbm
        
        with open("listings_dbm.csv","wb") as file:
            for key in listings_dbm:
                file.write("\n")
                file.write(key+",",)
                for value in listings_dbm[key]:
