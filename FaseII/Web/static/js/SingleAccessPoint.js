...
function MobileGuessedSingleAcessPoint(position,values,user){
    ...
    this.showAP = function(){
        ...
        if (this.values == 1.0){
            color = 'green';
           // this.circle = L.circle(this.position,5*tagusMap.metersConvertion);
           // tagusMap.map.addLayer(this.circle);
        ...
        this.marker = L.marker(this.position, {icon: positionMarker})
        
        this.marker.bindPopup("<p> User: "+this.user+"</p>");
        
        tagusMap.map.addLayer(this.marker)
                              

    },
    this.clear = function(){
        tagusMap.map.removeLayer(this.marker)
        
        //if( !typeof( this.circle ) === "undefined"){
            //tagusMap.map.removeLayer(this.circle)
        //}
    }
    this.init()
}